...
`POST /api/sync` (push + pull w jednym round-tripie) i `GET /api/sync?since=` (pull-only, np. świeża instalacja). Scope na razie: `exercises` (własne usera, nie globalne -- klient ich nie tworzy), `workouts`, `workout_exercises`, `sets`. `routines`/`routine_items`/`body_weights` nie istnieją jeszcze jako encje -- dojdą do sync przy etapach 6/7.
  ...
  src/main/java/com/example/easygymbackend/
    ...
    sync/                        # etap 5 -- POST /api/sync (push+pull), GET /api/sync?since= (pull-only)
      ...
      dto/ExerciseSyncRecord.java, WorkoutSyncRecord.java, WorkoutExerciseSyncRecord.java, SetSyncRecord.java,
          SyncBatch.java, SyncPushRequest.java, SyncPullResponse.java
